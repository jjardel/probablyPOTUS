...
SELECT
    ...
    CASE WHEN (created_at - timestamp_last_tweet < '5 minutes' OR timestamp_next_tweet - created_at < '5 minutes')
              AND tweet_source = 'android' -- these are likely tweets coming from his device
              AND text NOT LIKE '"@%' -- he often RTs lots of tweets in a row, and starts by copying the text of the tweet
